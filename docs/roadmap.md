...
| Body metric goals (target weight/body fat + progress-to-goal indicator) | `BodyMetricEntry` has no notion of a target, only logged history | Fast (single additive field/entity + a computed display value, no migration risk beyond one new column) |
